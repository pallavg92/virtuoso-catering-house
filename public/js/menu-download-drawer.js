...
(function () {
  ...
  function openDrawer(e) {
    ...
    // The scroll popup on /wedding-menu opens this drawer, and it must not
    // stay sitting behind it — closing the drawer would then restore page
    // scrolling with a popup still on screen. Mirrors inquire-drawer.js.
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // A menu download is a lead like any other, so it carries the same source
    // record. Captured on arrival by attribution.js.
    if (typeof window.vchAttribution === 'function') {
      payload.attribution = window.vchAttribution();
    }
    ...
    try {
      ...
      setStatus(
        (data.message || 'Thank you — your download will begin shortly.') +
          ' Opening the enquiry form next.',
        'success'
      );
      form.reset();
      if (data.downloadUrl) triggerDownload(data.downloadUrl);

      // A download is the start of a conversation, not the end of one, so the
      // visitor is handed straight to the enquiry form rather than left on a
      // success message. Everything already answered is carried across, which
      // leaves one question outstanding instead of six. The delay lets the
      // browser start the file and lets the success line be read first.
      setTimeout(function () {
        if (typeof window.vchOpenInquiry !== 'function') return;
        closeDrawer();
        window.vchOpenInquiry({
          name: payload.name,
          email: payload.email,
          phone: payload.phone,
          eventDate: payload.eventDate,
          guestCount: payload.guestCount,
          // Only on the wedding menu itself. The same drawer serves /services
          // and the landing pages, where the occasion is genuinely unknown and
          // guessing it would put a wrong answer in front of the visitor.
          eventType: document.body.classList.contains('page-wedding-menu') ? 'Wedding' : ''
        });
      }, 1400);
