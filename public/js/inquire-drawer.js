...
(function () {
  ...
  // Exposed so another flow can hand a visitor straight into this form. The
  // menu download uses it: once the PDF is on its way it reopens here with the
  // details already given, so the only thing left to answer is the event type.
  // Prefill never overwrites something the visitor has already typed.
  window.vchOpenInquiry = function (prefill) {
    openDrawer();
    if (prefill) {
      Object.keys(prefill).forEach((name) => {
        const field = form.querySelector('[name="' + name + '"]');
        if (field && !field.value && prefill[name]) field.value = prefill[name];
      });
    }
    const firstEmpty = Array.prototype.find.call(
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // Where this lead came from, captured on arrival by attribution.js. Sent
    // with the enquiry so a deal that closes weeks later can still be traced
    // back to the click, campaign or page that produced it.
